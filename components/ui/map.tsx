...
function MapPolyline({
    ...
    return (
        <LeafletPolyline
            className={cn(
                "fill stroke-foreground stroke-2",
                className
            )}
            {...props}
        />
    ...
    return (
        <MapControlContainer className={cn(position, className)}>
            <Button
                ...
                aria-label={
                    isFullscreen ? "Ieșire din ecran complet" : "Intrați pe ecran complet"
                }
                title={isFullscreen ? "Ieșire din ecran complet" : "Intrați pe ecran complet"}
                className="border"
                {...props}>
    ...
    return (
        <MapControlContainer className={cn(position, className)}>
            <Button
                type="button"
                size="icon-sm"
                variant={location ? "mapButtonDefault" : "mapButtonSecondary"}
                ...
                aria-label={
                    isLocating
                        ? "Localizare..."
                        : location
                          ? "Opriți urmărirea locației"
                          : "Începeți urmărirea locației"
                }
                className="border"
                {...props}>
                {isLocating ? (
...
function MapDrawShapeButton<T extends Draw.Feature>({
    ...
    return (
        <Button
            type="button"
            size="icon-sm"
            aria-label={`Plasează ${drawMode}`}
            title={`Plasează ${drawMode}`}
            className={cn("border", className)}
            variant={isActive ? "mapButtonDefault" : "mapButtonSecondary"}
...
function MapDrawActionButton<T extends EditToolbar.Edit | EditToolbar.Delete>({
    ...
    return (
        <Button
            type="button"
            size="icon-sm"
            aria-label={`${drawAction === "edit" ? "Editează" : "Șterge"} forme`}
            title={`${drawAction === "edit" ? "Editează" : "Șterge"} forme`}
            variant={isActive ? "mapButtonDefault" : "mapButtonSecondary"}
...
function MapDrawEdit({
    ...
    useEffect(() => {
        ...
        L.drawLocal.edit.handlers.edit.tooltip = {
            text: "Trageți de ghidaje sau marcaje pentru a edita.",
            subtext: "",
        }
